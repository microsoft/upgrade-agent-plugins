...
- **Primary keys and foreign keys**: Check `OnModelCreating` / Fluent API configuration before annotating PK/FK properties. Foreign key nullability determines relationship optionality — `int? OrderId` means an optional relationship, `int OrderId` means required. Never change FK nullability based on compiler warnings alone; verify the intended relationship type first. Getting this wrong generates migrations that silently change relationship semantics in the database.
