...
- **Intermediate 5 → 6 hop on the way to 7** (`targetTypeScriptVersion` is 7.x): **do NOT add `ignoreDeprecations`.** The deprecated options are fully removed in TypeScript 7.0, so the suppression only hides work you will be forced to do during the next hop. Fix the underlying deprecation now using the recipes above.
