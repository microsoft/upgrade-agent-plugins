...
2. **Runtime validation — REQUIRED, do not skip.** Call `typescript_validate_runtime` with `rootDirectory`, `packageDirectory`, and `sessionId`, then follow [runtime-validation.md](./runtime-validation.md). It is what catches runtime-only regressions the compiler cannot see. Skipping forces the workflow into the `inconclusive` bucket on dashboards and masks those regressions.
