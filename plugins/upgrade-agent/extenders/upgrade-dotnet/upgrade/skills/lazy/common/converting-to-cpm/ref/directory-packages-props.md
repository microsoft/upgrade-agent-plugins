...
If the CLI template is not available, create the file manually with `ManagePackageVersionsCentrally` set to `true` and `<PackageVersion>` entries in an `<ItemGroup>`.
...
```xml
<PackageReference Include="PackageA" VersionOverride="9.0.0" />
```
