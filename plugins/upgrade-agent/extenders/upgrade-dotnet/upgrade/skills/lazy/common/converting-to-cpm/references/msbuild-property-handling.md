...
Search all project files, `.props`, and `.targets` files in scope for references to the property name. If it appears only in `PackageReference` version attributes, it is safe to remove after inlining.
...
After restore and build succeed (step 8), remove property definitions the user chose to inline. Verify each has zero remaining references before removing. Preserve non-versioning properties in the same file.
