...
description: >
  ...
  Use when adopting NRTs in existing codebases, performing file-by-file or project-wide
  nullable migration, fixing CS8602/CS8618/CS86xx warnings, annotating APIs for nullability,
  cleaning up null-forgiving operators, or upgrading dependencies with new nullable annotations.
  Also use when the user asks to "enable nullable", "turn on NRTs", "annotate for nullability",
  "fix nullable warnings", or "migrate to nullable reference types".
...
> 🛑 **Do not update NuGet packages.** Do not upgrade dependencies to "get better nullable annotations" or to resolve oblivious-library warnings. Package updates are a separate change with their own risk surface. Only update packages if the user explicitly requests it.
...
4. If the project targets `< .NET 5.0` or `< .NET Standard 2.1`, check whether nullable attributes (`[NotNullWhen]`, `[MaybeNull]`, etc.) from `System.Diagnostics.CodeAnalysis` are available. If not, add the `Nullable` polyfill package from NuGet or define the attributes internally before proceeding — otherwise Step 5 will fail with missing type errors.
5. Check whether `<Nullable>` is already set. If it is set to `enable`, skip to Step 5 to audit remaining warnings.
6. Determine the project type — this shapes annotation priorities throughout the migration:
...
- **Generic types (`Repository<T>`, `Cache<T>`, etc.)**: `T?` on an unconstrained generic means different things for reference and value types — for value types it changes the signature to `Nullable<T>` (binary-breaking). Consider adding `where T : class` (enables `T?`), `where T : notnull` (disallows null and `Nullable<T>`), or `where T : struct` to give the compiler accurate information. Adding a constraint to a **public** generic type is a breaking change for consumers using types that don't meet the constraint — for public APIs, prefer `[MaybeNull]` / `[NotNull]` attributes over narrowing constraints. For internal types, constraints are preferred.
- **`async` return types**: `Task<T?>` vs `Task<T>` — the `Task` itself is never null, but the result can be. When an async method can return null, annotate the result type: `Task<string?>`, not `Task<string>`. Do not annotate the `Task` itself as nullable.
- **Collection element nullability**: `List<string>` vs `List<string?>` — decide whether the collection can contain null elements. Note that reflection and `System.Text.Json` cannot distinguish these at runtime (they share the same `List<string>` type in IL), so element nullability is compiler-enforced only.
- **Empty string vs null**: `string` properties that should "never be null but might be empty" should remain `string` (non-nullable), not `string?`. Empty string and null are distinct states — do not conflate them. Initialize with `= string.Empty` or `= ""` if needed.
- **P/Invoke and COM interop signatures**: Do not change parameter types in `[DllImport]` / `[LibraryImport]` method signatures. For reference-type parameters, adding `?` is metadata-only and safe from a marshaling perspective, but may misrepresent the native contract. For value-type parameters in interop, adding `?` changes the type to `Nullable<T>`, which breaks marshaling. When in doubt, add `#nullable disable` to interop declaration files and leave them unannotated.
...
| Generated code produces warnings | Generated files (`.designer.cs`, `.g.cs`, `.g.i.cs`, `Reference.cs`, or files with `<auto-generated>` comments) are excluded from nullable analysis automatically. If warnings persist, add `#nullable disable` at the top of the generated file or configure `.editorconfig` with `generated_code = true`. Do not spend tokens annotating code that will be overwritten by a generator |
