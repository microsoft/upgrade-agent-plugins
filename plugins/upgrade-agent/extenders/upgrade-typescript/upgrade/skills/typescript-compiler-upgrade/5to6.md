# TypeScript 6.0 Migration (from 5.x)

This guide covers the 5.x hops and the 5.x → 6.0 hop. It documents the breaking changes accumulated across TypeScript 5.0 through 5.9 (you encounter all of them on the way to 6.0) plus the 6.0 migration itself.

## Install

**npm:** `npm install -D typescript@6`
**yarn:** `yarn add -D typescript@6`
**pnpm:** `pnpm add -D typescript@6`

## Upgrade Checklist

1. Run the `ts5to6` migration tool: `npx -y @andrewbranch/ts5to6 --fixBaseUrl tsconfig.json` and `npx -y @andrewbranch/ts5to6 --fixRootDir tsconfig.json`
2. Install TypeScript 6.
3. Call `typescript_compile_package` to check for errors.
4. Fix errors based on what you encounter — **start with default changes**, as they cause the majority of errors (see the 6.0 migration table below). Re-compile after each batch until the build is clean.
...
| `types`                        | `["*"]` (all @types)  | `[]` (none)              | Add `"types": ["node"]` or needed types                         |
...
**Error: `Cannot find name 'process'`** (or other missing globals like `Buffer`, `describe`, `it`):
...
Add the packages you need: `"node"`, `"jest"`, `"mocha"`, etc. Use `"types": ["*"]` for old auto-include behavior.
...
## Error: `Option 'target' with value 'es5' is deprecated`
...
## New strict-mode errors (e.g., `'x' is possibly 'null'`)
...
Or fix the type errors (recommended).

## Error: `'module' declarations can only be used in TypeScript files` (in namespace declarations)
...
`outFile` is removed in TypeScript 7. Abort and tell the user they need to use their own bundler (esbuild, Rollup, webpack) now:
...
AMD, SystemJS, and UMD module formats are removed. Abort and tell the user they need to pick a new module format / use a bundler:
...
## Error: `Option 'esModuleInterop' cannot be set to 'false'` (or `allowSyntheticDefaultImports`)
...
## Error: `Option 'downlevelIteration' is deprecated`
...
The point of this migration is to surface and fix real breaking changes. **Do not paper over errors with suppression escape hatches.** Do not introduce any of the following as a "fix":

- **`// @ts-ignore` / `// @ts-expect-error` / `// @ts-nocheck`** added to silence new errors from the upgrade.
- **Casts to `any`** (e.g. `x as any`, `as unknown as T`) or widening a type to `any` to make an error go away.
- **Flipping `tsconfig.json` strictness flags** (`strict`, `noImplicitAny`, `strictNullChecks`, `skipLibCheck`, etc.) from their existing value just to silence upgrade errors.

If a real fix is genuinely out of scope, revert the upgrade rather than committing a suppression.

## `ignoreDeprecations` — Depends on the Target Version

Whether `ignoreDeprecations` is acceptable depends on whether TypeScript 6 is the **final** target of this migration or an **intermediate** hop on the way to TypeScript 7.

- **Terminal 5 → 6 upgrade** (`targetTypeScriptVersion` is 6.x): adding `"ignoreDeprecations": "6.0"` is acceptable to defer fixing deprecation warnings, since 6.x will keep accepting the deprecated options.
...
- **Intermediate 5 → 6 hop on the way to 7** (`targetTypeScriptVersion` is 7.x or `@typescript/native-preview`): **do NOT add `ignoreDeprecations`.** The deprecated options are fully removed in TypeScript 7.0, so the suppression only hides work you will be forced to do during the next hop. Fix the underlying deprecation now using the recipes above.

If an existing `ignoreDeprecations` entry is already in `tsconfig.json` before this upgrade, leave it as-is — do not remove it as part of this hop.

## Troubleshooting

Two recurring breakages appear when upgrading projects to TypeScript 6. Apply these
fixes when you see the matching errors after a successful install.

### TS2307 on private package subpaths

**Symptom:** After switching `moduleResolution` to `bundler` (or `node16` /
`nodenext`), imports that reach into a package's internals fail:

```
src/utils/zod.ts(3,26): error TS2307: Cannot find module 'zod/v4/core/util.cjs'
```

Under these resolvers TypeScript honors the package's `exports` map, so subpaths
that the package does not publish are no longer reachable — even if the file
exists on disk.

**Fix:**

Find the public `exports`-mapped path that exposes the same symbol. Every
well-maintained package publishes types through its `exports` map — the private
subpath exists only for the package's own internal use. Look in order:

1. Check the package's top-level export (`import { X } from 'pkg'`).
2. Check documented sub-entry-points in the `exports` field of `package.json`
   (e.g. `zod/v4`, `zod/v4/core`).
3. Run `node -e "console.log(Object.keys(require('pkg/package.json').exports))"` to
   see all published paths at once.
4. Search the package's type declaration files for the symbol name to locate which
   public entry re-exports it.

Do **not** fall back to inlining a local copy of the type. That creates drift and
hides the real fix. If you cannot locate a public re-export after the steps above,
open the package's repo and search its `exports` map directly — it is almost always
there under a different entry point than the one the code was using.

Do not work around this by disabling `exports` resolution or downgrading
`moduleResolution`.

**Real-world example — `zod` private subpath (`cli-microsoft365`):**

The file used `EnumLike` from the private path `zod/v4/core/util.cjs`:

```ts
// ❌ broken under moduleResolution: bundler — private subpath, not in exports map
import { EnumLike } from 'zod/v4/core/util.cjs';
```

Looking at `zod`'s `exports` map and type declarations showed that `zod/v4/core`
re-exports the entire `util` namespace publicly:

```ts
// ✅ correct fix — use the public exports-mapped entry point
import { JSONSchema, util } from 'zod/v4/core';
type EnumLike = util.EnumLike;
```

### TS2593 / TS2304 for test-runner globals (`describe`, `it`, `before`, ...)

**Symptom:** Spec files fail to compile with messages like:

```
Cannot find name 'describe'. ... Try `npm i --save-dev @types/mocha` and then
add 'mocha' to the types field in your tsconfig.
```

TypeScript 6 no longer auto-includes every `@types/*` package found in
`node_modules`. Once a `types` field is present (or the default behavior
changes), only the listed packages are loaded.

**Fix — apply reactively, only when these errors appear:**

1. Confirm the relevant `@types/*` package is installed (`@types/mocha`,
   `@types/jest`, `@types/node`, etc.). Install it if missing.
2. Add a `types` entry to the `tsconfig.json` that covers the failing files
   (often a test-specific `tsconfig.test.json`). Include `node` plus the test
   runner used by the project, for example:
   ...
   Use `"jest"` instead of `"mocha"` for Jest projects. Only add runners that
   are actually in `devDependencies`.

Do **not** add a `types` field preemptively when a test runner is merely
present in `devDependencies` — wait for TS2593/TS2304 to surface, because
adding `types` narrows global type loading and can introduce its own
regressions.
